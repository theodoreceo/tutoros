...
export async function renderHistoryPage() {
  await ensureLoaded(['history_log', 'students', 'groups']);
  ...
  el.innerHTML = entries.map(e => `
    <div style="display:flex;align-items:flex-start;gap:12px;padding:12px 16px;border-bottom:1px solid var(--border)" id="hlog-${e.id}">
      ...
      ${e.undo_data && role.isOwner ? `<button class="btn btn-sm btn-danger" data-action="undoHistoryEntry" data-id="${esc(e.id)}"><i class="ti ti-rotate-left"></i> Отменить</button>` : ''}
    </div>`).join('');
}
